...
retrain = st.sidebar.button('Train / Retrain Model')
...
if retrain:
    with st.spinner('Training model (may take a while)...'):
        model, scaler, stats = train_and_save_model(force_retrain=True)
    st.session_state['model'] = model
    st.session_state['scaler'] = scaler
    st.sidebar.success('Model trained and saved.')
    if stats:
        st.sidebar.write('Training stats:')
...
def train_and_save_model(force_retrain=False):
    ...
    stats = {
        'lr_mae': lr_mae, 'rf_mae': rf_mae, 'lr_r2': lr_r2, 'rf_r2': rf_r2, 'best': 'RandomForest' if rf_r2>=lr_r2 else 'LinearRegression'
    }
...
if st.button('Generate Personalized Meal Plan'):
    ...
    if exercise_hrs < 2.5:
        ...
        st.write("- Current: {:.1f} hours/week".format(exercise_hrs))
    ...
    if sleep_hrs < 7:
        ...
        st.write("- Current: {:.1f} hours/day".format(sleep_hrs))
    ...
    st.write("""
    Your daily nutrition targets and actuals are broken down as follows:
    """)
    ...
    with col1:
        ...
        st.write("Target: {:.0f} kcal".format(pred_cal))
    ...
    with col2:
        ...
        st.write("Target: {:.0f}g (25% of calories)".format(protein_target))
    ...
    with col3:
        ...
        st.write("Target: {:.0f}g (50% of calories)".format(carbs_target))
